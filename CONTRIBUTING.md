...
```jsonc
{
  ...
  "notes": [                                // (선택) "기획에서 고려한 점" — 접이식으로 표시
    "**문제 정의:** 어떤 사용자의 어떤 불편에서 출발했는지",
    "**핵심 컨셉:** 이 제품만의 포지셔닝/원칙",
    "**타깃 사용자:** 누구를 위해 만들었는지"
  ],
  "notesTitle": "💡 <b>기획에서 고려한 점</b>"  // (선택) 접이식 제목. 생략 시 이 값이 기본
}
```

> `notes` 는 카드 하단에 접이식(`<details>`)으로 들어갑니다. **개발이 아니라 기획·아이디어 기여**(왜 만들었나, 누구를 위해, 어떤 원칙으로)를 보여주는 칸입니다. `notesTitle` 로 제목을 바꿀 수 있고(예: `🧠 <b>설계에서 고려한 점</b>`), 둘 다 없으면 생략됩니다.
